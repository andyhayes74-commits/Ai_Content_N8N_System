...
Smith v1 is the first supervisor-agent design already captured in:

```text
docs/smith_supervisor_agent_design_brief.md
```
...
```text
Memory:
tool_qa_delivery frequently fails with invalid_json when model output includes markdown fences.

Embedded behaviour:
When invalid_json occurs in QA, Smith checks for markdown fences, repair count, and known repair success before recommending an action.
```
