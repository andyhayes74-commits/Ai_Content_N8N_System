...
def common_start(name):
    return [
        ...
        node(2, "Normalize Input", "n8n-nodes-base.code", [220, 0], {"jsCode": "const body=$json.body??$json;const headers=$json.headers??{};return [{json:{...body,job_id:body.job_id||$json.job_id||'',mode:body.mode||'live',agent_secret:headers['x-agent-secret']||headers['X-Agent-Secret']||''}}];"}, 2),
...
def create_new_drive_project_folder():
    ...
    nodes = common_start(name) + [
        ...
        node(7, "Create Folder in Google Drive", "n8n-nodes-base.httpRequest", [1100, 0], {"method": "POST", "url": "https://www.googleapis.com/drive/v3/files", "sendHeaders": True, "headerParameters": {"parameters": [{"name": "Authorization", "value": "={{'Bearer ' + ($env.GOOGLE_DRIVE_ACCESS_TOKEN || 'GOOGLE_DRIVE_AI_CONTENT')}}"}, {"name": "Content-Type", "value": "application/json"}]}, "sendBody": True, "specifyBody": "json", "jsonBody": "={{JSON.stringify($json.drive_request)}}", "options": {"timeout": 120000}}, 4),
...
def simple_http_marker_workflow(name, event_type, message, output=False):
    nodes=common_start(name)+[
        node(5,"Load Job","n8n-nodes-base.postgres",[660,0],{"operation":"executeQuery","query":"=SELECT id::text AS job_id, drive_root_folder_id FROM content_jobs WHERE id=NULLIF('{{$json.job_id}}','')::uuid;"}),
        node(6,"Google Drive API Placeholder","n8n-nodes-base.httpRequest",[880,0],{"method":"GET","url":"https://www.googleapis.com/drive/v3/files","sendHeaders":True,"headerParameters":{"parameters":[{"name":"Authorization","value":"={{'Bearer ' + ($env.GOOGLE_DRIVE_ACCESS_TOKEN || 'GOOGLE_DRIVE_AI_CONTENT')}}"}]},"options":{"timeout":120000}},4),
